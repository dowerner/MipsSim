...
namespace MipsSim.View
{
    ...
    /// </summary>
    public partial class MemoryEditView : Window
    {
        public MemoryEditView()
        {
            InitializeComponent();
            KeyDown += MemoryEditView_KeyDown;  // catch keyboard
        ...
        // enable keyboard control for window
        private void MemoryEditView_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Escape) Close();
            if (e.Key == Key.Enter)
            {
                ((MemoryEditViewModel)DataContext).Save();
                Close();
            }
        }
